function firstSave() {
    ...
    for (const item of items)
        itemData.push([item.getItemName(), item.amt, item.placed, item.astralAmt]);
    saveSetup();
...
function loadData() {
    //Load Items
    let loadItemData = JSON.parse(localStorage.getItem("itemData"));
    console.log(loadItemData);
    for (let i = 0; i < loadItemData?.length; i++) {
        let temp = items[locateItemIndex(loadItemData[i][0])];
    ...
    //Load Setups + Layouts
    {
        ...
        for (let i = 0; i < tempLayouts?.length; i++)
            for (let j = 0; j < tempLayouts[i]?.length; j++)
    ...
    document.getElementById("layout" + 2 + "Items").innerHTML = layouts[2].length + "/75";
    {
        ...
        for (let i = 0; i < setupNames?.length; i++)
    ...
    //Everything Else
    const reqs = JSON.parse(localStorage.getItem("setupReqs"));
    hasDropper = reqs[0];
    hasFurnace = reqs[1];
    setSetupValue();
...
function saveLayouts() {
    ...
    for (let i = 0; i < layouts.length; i++)
        for (let j = 0; j < layouts[i].length; j++)
